...
// In production (Vercel), set VITE_API_BASE_URL in the Vercel dashboard:
 ...
 *  In production set VITE_MEDIA_BASE_URL = https://your-render-service.onrender.com
 */
...
async function refreshAccessToken() {
  ...
  try {
    ...
    const response = await axios.post<{ access: string }>(
      `${API_BASE_URL.replace(/\/api\/?$/, '')}/api/auth/token/refresh/`,
      { refresh },
    )
