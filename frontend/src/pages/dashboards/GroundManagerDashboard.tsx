// ================================================================
// GROUND MANAGER DASHBOARD (pages/dashboards/GroundManagerDashboard.tsx)
// ================================================================
// PURPOSE: Dashboard for the Ground Manager (Site Supervisor) role.
//
// DESIGNED FOR: Mobile-first (field workers on phones/tablets)
//
// SECTIONS:
//   - Today's tasks overview
//   - Active sheet assignments (click to view/edit)
//   - Quick data entry for site observations
//   - Notification center
//   - Site progress photos (placeholder)
//
// DATA: Calls user-sheets, assignments, notifications APIs
// ROLE ACCESS: Ground Manager only
// ================================================================
...
// ─── GROUND MANAGER DASHBOARD (Mobile-First Design) ───
// This dashboard is designed for site supervisors using phones/tablets.
const GroundManagerDashboard: React.FC = () => {
  const navigate = useNavigate();
  useAuth();                                      // Auth context (ensures login)
  const { socket } = useSocket();
  const [loading, setLoading] = useState(true);
  const [tasks, setTasks] = useState<Assignment[]>([]);                  // Site tasks
  const [, setNotifications] = useState<AppNotification[]>([]); // Notifications (stored for future use)
  const [unreadCount, setUnreadCount] = useState(0);
  const [quickAnswers, setQuickAnswers] = useState<Record<string, Record<string, string>>>({}); // Quick data entries
  ...
  // When the admin pushes a new task, auto-reload the dashboard
  useEffect(() => {
    if (!socket) return;
  ...
  // Split tasks into pending (active) and completed groups for the UI
  ...
  const handleQuickSubmit = async (task: Assignment) => {
    try {
      ...
      if (Object.keys(toSend).length === 0) {
        toast.error('Fill in at least one cell');
        setSubmitting(null);
        return;
      }
  ...
  const menuItems = [
    { label: 'Dashboard', path: '/ground-manager', icon: <ConstructionIcon /> },
    { label: 'My Tasks', path: '/my-tasks', icon: <AssignmentIcon /> },
    { label: 'My Sheets', path: '/my-sheets', icon: <SendIcon /> },
  ];

  return (
    <DashboardLayout title="Ground Manager" menuItems={menuItems}>
      {loading ? (
        ...
        <Box>
          ...
          <Alert severity="info" sx={{ mb: 2 }}>
            ...
            <strong>Site Supervisor View</strong> — Fill in field data assigned by your admin. Your responses auto-populate the project sheets.
          </Alert>

          {/* Pending Tasks */}
          <Typography variant="h6" gutterBottom>
            <AssignmentIcon sx={{ mr: 1, verticalAlign: 'middle' }} />
            Site Tasks ({pendingTasks.length})
          </Typography>

          {pendingTasks.length === 0 ? (
            <Alert severity="success" sx={{ mb: 3 }}>No pending site tasks!</Alert>
          ) : (
              ...
              <Card key={task.id} sx={{ mb: 2, border: task.priority === 'URGENT' ? '2px solid #d32f2f' : '1px solid #e0e0e0' }}>
                <CardContent>
                  <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
                    ...
                    <Button size="small" startIcon={<OpenIcon />} onClick={() => navigate(`/sheet/${task.sheetId}`)}>Open Sheet</Button>
                  </Box>
                  ...
                  {task.question && (
                    <Alert severity="warning" sx={{ my: 1 }}><strong>Instructions:</strong> {task.question}</Alert>
                  )}
              ...
              <List dense>
                {completedTasks.slice(0, 10).map(task => (
                  <ListItem key={task.id} divider>
                    <ListItemIcon>
                      {task.status === 'APPROVED' ? <CheckIcon color="success" /> : <PendingIcon color="info" />}
                    </ListItemIcon>
                    <ListItemText
                      primary={`${task.sheet?.name} — ${task.assignmentType}`}
